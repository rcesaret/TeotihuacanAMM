...
PlazaPoly_sf = st_as_sf(Teo_ArchPoly_Data) %>% filter(XM_group_f == "Plaza")
  ...
  my_breaks <- c(0, 40, 80, floor(max(Teo_smooth$v, na.rm=T)))
...
f4a <- ggplot() +
  ...
  annotate("label", x = 3000, y = 70, label = "atop(bold(italic(PopDens) == 154.4 * italic(e) ^ (-0.00083 * ~italic(Dist))),bold(italic(pseudo~R^2) == 0.967 ~~~~~~ italic(n) == 8 ))", parse = TRUE, color ="black", size=4)+
