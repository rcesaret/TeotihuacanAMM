...
```{r include=FALSE,echo=F,message=FALSE,warning=FALSE,results = 'hide'}
#Estimate the model
exp_nls_fit <- nls.multstart::nls_multstart(PctRow_Area_ArchFeat ~ expfunc(MedRingDist, b1, b2),
                             data = TeoRings,
...
pp5 = ModelPredFrame(exp_nls_fit,TeoRings,"MedRingDist","PctRow_Area_ArchFeat")
...
exp_nls_fit_results = RegressionResultsTab(exp_nls_fit, TeoRings, TeoRings$MedRingDist, TeoRings$PctRow_Area_ArchFeat, model_type="nls", coef="both", residplots=TRUE, residhists=TRUE)
...
f4c <- ggplot() +
  ...
  geom_line(data = pp5, aes(x=MedRingDist, y=PctRow_Area_ArchFeat.p), inetype = "solid", color="purple", size=1.3) +
  ...
  annotate("label", x = 2875, y = 0.62, label = "— Arch Features % of Ring Area", color ="purple", size=5, face = "bold")+
  annotate("label", x = 2875, y = 0.51, label = "atop(bold(italic(PctArchFeat) == 0.825 * italic(e) ^ (-0.0008 * ~italic(Dist))),bold(italic(pseudo~R^2) == 0.988 ~~~~~~ italic(n) == 9 ))", parse = TRUE, color ="purple3", size=4)+
    
  #annotate("label", x = 2875, y = 0.4, label = "— % of Total CC Feat Area", color ="cyan4", size=5, face = "bold")+
  annotate("label", x = 2875, y = 0.4, label = "— CC Feat % of Ring Area", color ="blue", size=5, face = "bold")+
...
f4b <- ggplot(areas, aes(x=Ring, y=percentage, fill=LandUse)) + 
...
f4d <- ggplot(areas, aes(x=Ring, y=percentage, fill=Class)) + 
    geom_area(alpha=0.6 , size=1, colour="black",stat="identity")+
...
f5a <- ggplot(data = TeoRings, aes(x = PctRow_Area_CCStr, y = PctRow_DStrArea_Elite)) +
...
```{r}
pwr_nls_fit_results = RegressionResultsTab(pwr_nls_fit, TeoRings, TeoRings$PctRow_Ring_DStrArea_Low, TeoRings$PctRow_Area_CCFeat_AoD, model_type="nls", coef="both", residplots=TRUE, residhists=TRUE)
...
```{r Define simple power function, include=FALSE,echo=F,message=FALSE,warning=FALSE,results = 'hide'}
#logfunc <- function(PctRow_Area_CCFeat_AoD, b1, b2) {
#  b1 * PctRow_Area_CCFeat_AoD + b2
#}
#pwr_nls_fit <- nls.multstart::nls_multstart(PctRow_Area_ArchFeat ~ pwrfunc(log(PctRow_Area_CCFeat_AoD), b1, b2),
#                             data = TeoRings,
#                             start_lower = c(b1=-10, b2=-10),
#                             start_upper = c(b1=10, b2=10),
#                             iter = 500,
#                             supp_errors = "Y")

#pp3 = ModelPredFrame(pwr_nls_fit,TeoRings,"PctRow_Area_CCFeat_AoD","PctRow_Area_ArchFeat")
#pwr_nls_fit_results = RegressionResultsTab(pwr_nls_fit, TeoRings, log(TeoRings$PctRow_Area_CCFeat_AoD), #TeoRings$PctRow_Area_ArchFeat, model_type="nls", coef="both", residplots=TRUE, residhists=TRUE)

#pwr_nls_fit_results[[1]]
#pwr_nls_fit_results[[2]]
#pwr_nls_fit_results[[3]]

mod = lm(PctRow_Area_ArchFeat ~ log(PctRow_Area_CCFeat_AoD), TeoRings)
summary(mod)
pp31 = ModelPredFrame(mod,TeoRings,"PctRow_Area_CCFeat_AoD","PctRow_Area_ArchFeat")
#mod_results = RegressionResultsTab(mod, TeoRings, log(TeoRings$PctRow_Area_CCFeat_AoD), TeoRings$PctRow_Area_ArchFeat, model_type="nls", coef="both", residplots=TRUE, residhists=TRUE)
#mod_results[[3]]
```
...
f5d <- ggplot() +
  ...
  geom_line(data = pp31, aes(x=PctRow_Area_CCFeat_AoD, y=PctRow_Area_ArchFeat.p), inetype = "solid", color="purple", size=1.3) +
...
f6b <- ggplot() +
  ...
  scale_color_manual(name = "Avg DStr Area", values = c("black","green2","gold","darkorange", "red"), labels=c('Total', 'High','Intermediate','Uncertain', 'Low x 10'))+#"orange2",
...
```{r}
Fig6 <- plot_grid(f6a, f6b, f6c, f6c, labels = "AUTO", label_size = 30, hjust=0)#f6c, f6d,
